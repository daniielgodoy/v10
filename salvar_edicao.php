...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $mysqli->query("DELETE FROM pedido_itens WHERE pedido_id = $pedido_id");
    ...
    $res = $mysqli->query("SELECT cliente_nome, cliente_numero FROM pedidos WHERE id = $pedido_id")->fetch_assoc();
    $cliente_nome = $res["cliente_nome"];
    $cliente_numero = $res["cliente_numero"];

    // ========================
    // Gerar PDF (igual bot.js)
    // ========================
...
cliente_nome = \"" . addslashes($cliente_nome) . "\"
output_path = r\"" . addslashes($pdfPath) . "\"
...
for item in carrinho_ordenado:
    ...
    tipo_leg = mapa_tipos.get(item.get('tipo','').lower(), 'Peça')
    nome = f\"{tipo_leg} {(item.get('nome') or item.get('modelo') or 'DESCONHECIDO').upper()}\"

    qtd = int(item.get('quantidade', 1))
    subtotal = float(item['preco']) * qtd
    data.append([str(qtd), nome + ((f' ({detalhe})') if detalhe else ''), 'R$ %.2f' % subtotal])
    total += subtotal
...
";
    ...
    $pyFile = __DIR__ . "/tmp_edit_" . time() . ".py";
