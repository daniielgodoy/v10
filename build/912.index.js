...
;// CONCATENATED MODULE: ./node_modules/music-metadata/lib/asf/GUID.js
...
class GUID {
    ...
    static decode(objectId, offset = 0) {
        ...
        const guid = `${view.getUint32(0, true).toString(16)}-${view.getUint16(4, true).toString(16)}-${view.getUint16(6, true).toString(16)}-${view.getUint16(8).toString(16)}-${(0,uint8array_extras/* uint8ArrayToHex */.EY)(objectId.slice(offset + 10, offset + 16))}`;
    ...
    static encode(str) {
        ...
        bin.set((0,uint8array_extras/* hexToUint8Array */.AS)(str.slice(19, 23)), 8);
        bin.set((0,uint8array_extras/* hexToUint8Array */.AS)(str.slice(24)), 10);
        return bin;
    }
    constructor(str) {
        this.str = str;
...
/* harmony default export */ const asf_GUID = (GUID);
...
;// CONCATENATED MODULE: ./node_modules/music-metadata/lib/asf/AsfUtil.js
...
function parseUnicodeAttr(uint8Array) {
    return Util/* stripNulls */.F6(Util/* decodeString */.sc(uint8Array, 'utf-16le'));
}
...
function parseDWordAttr(buf, offset = 0) {
    return lib.UINT32_LE.get(buf, offset);
}
function parseQWordAttr(buf, offset = 0) {
    return lib.UINT64_LE.get(buf, offset);
}
function parseWordAttr(buf, offset = 0) {
    return lib.UINT16_LE.get(buf, offset);
}
...
;// CONCATENATED MODULE: ./node_modules/music-metadata/lib/asf/AsfObject.js
// ASF Objects
...
class AsfContentParseError extends (0,ParseError/* makeUnexpectedFileContentError */.fO)('ASF') {
}
/**
...
const TopLevelHeaderObjectToken = {
    ...
    get: (buf, off) => {
        return {
            objectId: asf_GUID.fromBin(buf, off),
            objectSize: Number(lib.UINT64_LE.get(buf, off + 16)),
            numberOfHeaderObjects: lib.UINT32_LE.get(buf, off + 24)
            // Reserved: 2 bytes
        };
    }
};
...
class FilePropertiesObject extends State {
    get(buf, off) {
        return {
            fileId: asf_GUID.fromBin(buf, off),
            fileSize: lib.UINT64_LE.get(buf, off + 16),
            creationDate: lib.UINT64_LE.get(buf, off + 24),
            dataPacketsCount: lib.UINT64_LE.get(buf, off + 32),
            playDuration: lib.UINT64_LE.get(buf, off + 40),
            sendDuration: lib.UINT64_LE.get(buf, off + 48),
            preroll: lib.UINT64_LE.get(buf, off + 56),
            flags: {
                broadcast: Util/* getBit */.mh(buf, off + 64, 24),
                seekable: Util/* getBit */.mh(buf, off + 64, 25)
            },
            // flagsNumeric: Token.UINT32_LE.get(buf, off + 64),
            minimumDataPacketSize: lib.UINT32_LE.get(buf, off + 68),
            maximumDataPacketSize: lib.UINT32_LE.get(buf, off + 72),
            maximumBitrate: lib.UINT32_LE.get(buf, off + 76)
        };
    }
}
FilePropertiesObject.guid = asf_GUID.FilePropertiesObject;
/**
...
class StreamPropertiesObject extends State {
    get(buf, off) {
        return {
            streamType: asf_GUID.decodeMediaType(asf_GUID.fromBin(buf, off)),
            errorCorrectionType: asf_GUID.fromBin(buf, off + 8)
            // ToDo
        };
    }
}
StreamPropertiesObject.guid = asf_GUID.StreamPropertiesObject;
/**
...
class HeaderExtensionObject {
    ...
    get(buf, off) {
        ...
        return {
            reserved1: asf_GUID.fromBin(buf, off),
            reserved2: view.getUint16(16, true),
            extensionDataSize: view.getUint16(18, true)
        };
    }
}
HeaderExtensionObject.guid = asf_GUID.HeaderExtensionObject;
/**
...
async function readString(tokenizer) {
    const length = await tokenizer.readNumber(lib.UINT16_LE);
    return (await tokenizer.readToken(new lib.StringType(length * 2, 'utf-16le'))).replace('\0', '');
}
/**
...
async function readCodecEntry(tokenizer) {
    const type = await tokenizer.readNumber(lib.UINT16_LE);
    return {
        type: {
...
ContentDescriptionObjectState.guid = asf_GUID.ContentDescriptionObject;
...
ExtendedContentDescriptionObjectState.guid = asf_GUID.ExtendedContentDescriptionObject;
/**
...
class ExtendedStreamPropertiesObjectState extends State {
    get(buf, off) {
        const view = new DataView(buf.buffer, off);
        return {
            startTime: lib.UINT64_LE.get(buf, off),
            endTime: lib.UINT64_LE.get(buf, off + 8),
            ...
            flags: {
                reliableFlag: Util/* getBit */.mh(buf, off + 40, 0),
                seekableFlag: Util/* getBit */.mh(buf, off + 40, 1),
                resendLiveCleanpointsFlag: Util/* getBit */.mh(buf, off + 40, 2)
            },
            // flagsNumeric: Token.UINT32_LE.get(buf, off + 64),
...
ExtendedStreamPropertiesObjectState.guid = asf_GUID.ExtendedStreamPropertiesObject;
/**
...
MetadataObjectState.guid = asf_GUID.MetadataObject;
// 4.8	Metadata Library Object (optional, 0 or 1)
class MetadataLibraryObjectState extends MetadataObjectState {
}
MetadataLibraryObjectState.guid = asf_GUID.MetadataLibraryObject;
/**
...
class WmPictureToken {
    ...
    get(buffer, offset) {
        ...
        const format = new lib.StringType(index - 5, 'utf-16le').get(buffer, 5);
        while (view.getUint16(index) !== 0) {
            index += 2;
        }
        const description = new lib.StringType(index - 5, 'utf-16le').get(buffer, 5);
        return {
            type: ID3v2Token/* AttachedPictureType */.n5[typeId],
            format,
            description,
            size,
...
;// CONCATENATED MODULE: ./node_modules/music-metadata/lib/asf/AsfParser.js
...
const debug = src('music-metadata:parser:ASF');
 ...
 * - https://msdn.microsoft.com/en-us/library/windows/desktop/ee663575(v=vs.85).aspx
 */
class AsfParser extends BasicParser/* BasicParser */.s {
    async parse() {
        const header = await this.tokenizer.readToken(TopLevelHeaderObjectToken);
        if (!header.objectId.equals(asf_GUID.HeaderObject)) {
        ...
        do {
            ...
            switch (header.objectId.str) {
                ...
                case ExtendedContentDescriptionObjectState.guid.str: // 3.11
                    ...
                    await this.addTags(tags);
                    break;
                case asf_GUID.CodecListObject.str: {
                    const codecs = await readCodecEntries(this.tokenizer);
                    codecs.forEach(codec => {
                        this.metadata.addStreamInfo({
                            type: codec.type.videoCodec ? types/* TrackType */.S.video : types/* TrackType */.S.audio,
                ...
                case asf_GUID.StreamBitratePropertiesObject.str:
                    // ToDo?
                    await this.tokenizer.ignore(header.objectSize - HeaderObjectToken.len);
                    break;
                case asf_GUID.PaddingObject.str:
        ...
        do {
            ...
            switch (header.objectId.str) {
                ...
                case asf_GUID.PaddingObject.str:
                    ...
                    break;
                case asf_GUID.CompatibilityObject.str:
                    await this.tokenizer.ignore(remaining);
                    break;
                case asf_GUID.ASF_Index_Placeholder_Object.str:
                    await this.tokenizer.ignore(remaining);
                    break;
                default:
                    this.metadata.addWarning(`Ignore ASF-Object-GUID: ${header.objectId.str}`);
